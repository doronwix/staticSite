define(
    ...
    function (require) {
        ...
        function getPGSeenStatus() {
            var obj = JSONHelper.STR2JSON("HelpCenterViewModel:getPGSeenStatus", storageFactory.getItem(helpCenterStorageKey)) || { pgSeen: false };

            return obj.pgSeen;
        }

        function getPGInteractedStatus() {
            var obj = JSONHelper.STR2JSON("HelpCenterViewModel:getPGInteractedStatus", storageFactory.getItem(helpCenterStorageKey)) || { pgInteracted: false };

            return obj.pgInteracted;
        }

        function savePGInteractedStatus(statusSeen, statusInteracted) {
            var obj = JSONHelper.STR2JSON("HelpCenterViewModel:savePGInteractedStatus", storageFactory.getItem(helpCenterStorageKey)) || {};
        ...
        function getHelpCenterSeenStatus() {
            var obj = JSONHelper.STR2JSON("HelpCenterViewModel:getHelpCenterSeenStatus", storageFactory.getItem(helpCenterStorageKey)) || { helpCenterSeen: false };

            return obj.helpCenterSeen;
        }

        function saveHelpCenterSeenStatus(status) {
            var obj = JSONHelper.STR2JSON("HelpCenterViewModel:saveHelpCenterSeenStatus", storageFactory.getItem(helpCenterStorageKey)) || {};

            obj.helpCenterSeen = status;
            storageFactory.setItem(helpCenterStorageKey, JSON.stringify(obj));
        }

        var HelpCenterViewModel = general.extendClass(KoComponentViewModel, function (params) {
            var self = this,
                parent = this.parent,
                data = this.Data,
                soSubscriptions = [];
            ...
            function getLastSectionActive() {
                ...
                var obj = JSONHelper.STR2JSON("HelpCenterViewModel:getLastSectionActive", storageFactory.getItem(helpCenterStorageKey)) || {};
            ...
            function saveLastSectionActive(section) {
                ...
                var obj = JSONHelper.STR2JSON("HelpCenterViewModel:saveLastSectionActive", storageFactory.getItem(helpCenterStorageKey)) || {};
            ...
            function loadResources() {
                ...
                data.uploadDocsLink(content.uploadDocsLink);
            ...
            function setObservables() {
                ...
                data.uploadDocsLink = ko.observable(null);
                ...
                ko.computed(function () {
                    if (data.visible() && data.activeSection() === eHelpcSections.personalguide &&
                        ...
                        if (willBeVisible) {
                            if (data.pgPresent()) {
                                if (!(getPGSeenStatus() || getPGInteractedStatus())) {
                                    ...
                                    savePGInteractedStatus(true, stateObjectHelpc.get('pg-interacted'));
                                    data.pgSeen(true);
                                } else {
                ...
                },
                    {
                        unsubscribe: stateObjectHelpc.subscribe('pg-present', function (isPresent) {
                            ...
                            if (!data.visible()) {
                                ...
                                if (stateObjectHelpc.get('HidePersonalGuide') && !(data.pgSeen() || getPGInteractedStatus())) {
